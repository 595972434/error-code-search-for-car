...
def get_opensource_embeddings(texts: list[str], is_s2p=False, model=None, tokenizer=None):
...
if __name__ == '__main__':
    ...
    origin_model = AutoModel.from_pretrained('BAAI/bge-base-en-v1.5')
    origin_model.save_pretrained("./model/model")
    origin_model.eval()  # Set in inference mode. Turns off dropout etc.
    origin_model.to(DEVICE)  # Change which memory model resides
    ...
    model_state = origin_model.embeddings.state_dict()
    model_state['word_embeddings.weight'] = weights.T
    finetuned_model = origin_model
    finetuned_model.embeddings.load_state_dict(model_state)
    ...
    chunks, outs = get_opensource_embeddings(texts, model=finetuned_model, tokenizer=tokenizer)
    query_embedding, _ = get_opensource_embeddings(USER_QUERY, model=finetuned_model, tokenizer=tokenizer)
    relevant_page, page_no, X = find_relevant_page_from_document(query_embedding, passage_chunks=chunks)
    ...
    #
    rrf_score = rrf([keyword_top_5_matches_idx, X])

    print(f"Top 5 matches BM25: {keyword_top_5_matches_idx}")
    print(f"Top 5 matches semantic: {X}")
